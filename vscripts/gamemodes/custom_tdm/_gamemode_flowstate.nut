...
function _CustomTDM_Init() {
  ...
  AddCallback_OnClientConnected(void
...
void
function _RegisterLocation(LocationSettings locationSettings) {
  file.locationSettings.append(locationSettings)
  file.droplocationSettings.append(locationSettings)
}

LocPair
function _GetVotingLocation() {
  switch (GetMapName()) {
...
void
function _OnPropDynamicSpawned(entity prop) {
  file.playerSpawnedProps.append(prop)
}

int
function GetTDMState() {
  return file.tdmState
}

void
function Flowstate_ServerSaveChat() {
...
void
function SetTdmStateToNextRound() {
  file.tdmState = eTDMState.NEXT_ROUND_NOW
}

void
function SetTdmStateToInProgress() {
  file.tdmState = eTDMState.IN_PROGRESS
}

void
function SetFallTriggersStatus(bool status) {
  file.FallTriggersEnabled = status
}

LocPair
function _GetAppropriateSpawnLocation(entity player) {
  switch (GetGameState()) {
...
LocPair
function Flowstate_GetBestSpawnPointFFA() {
...
vector
function GetClosestEnemyToOrigin(vector origin, int ourTeam) {
  float minDist = -1
...
void
function DissolveItem(entity prop) {
  thread(void
    function (entity prop) {
      wait 4
...
void
function _OnPlayerConnected(entity player) {
  ...
  if (is1v1EnabledAndAllowed()) {
    void functionref() soloModefixDelayStart1 = void
    function (): (player) {
      ...
      try {
        player.Die(null, null, {
          damageSourceId = eDamageSourceId.damagedef_suicide
        })
      } catch (error) {
...
bool
function is1v1EnabledAndAllowed() {
...
void
function __HighPingCheck(entity player) {
  wait 12
...
void
function Flowstate_AppendBattleLogEvent(entity killer, entity victim) {
...
void
function Flowstate_SaveBattleLogToFile() {
...
void
function Flowstate_SaveBattleLogToFile_Linux() //Use parser
{
  if (file.battlelog.len() == 0) return
...
void
function _OnPlayerDied(entity victim, entity attacker,
  var damageInfo) {
  ...
  case eGameState.Playing:
    // Víctim
    void functionref() victimHandleFunc = void
    function (): (victim, attacker, damageInfo) {
    ...
    // Attacker
    void functionref() attackerHandleFunc = void
    function (): (victim, attacker, damageInfo) {
...
void
function PlayerKillStreakAnnounce(entity attacker, string doubleKill, string tripleKill) {
...
void
function CheckForObservedTarget(entity player) {
  OnThreadEnd(
    function (): (player) {
...
void
function _HandleRespawn(entity player, bool isDroppodSpawn = false) {
  if (!IsValid(player)) return
...
void
function ReCheckGodMode(entity player) {
  wait 0.1
...
void
function TpPlayerToSpawnPoint(entity player) {
...
void
function Flowstate_GrantSpawnImmunity(entity player, float duration) {
...
void
function WpnPulloutOnRespawn(entity player, float duration) {
...
void
function WpnAutoReload(entity player) {
  if (!IsValid(player)) return
...
void
function WpnAutoReloadOnKill(entity player) {
...
void
function SummonPlayersInACircle(entity player0) {
...
void
function __GiveWeapon(entity player, array < string > WeaponData, int slot, int select, bool isGungame = false) {
...
void
function GiveRandomPrimaryWeaponMetagame(entity player) {
...
void
function GiveRandomSecondaryWeaponMetagame(entity player) {
...
void
function GiveRandomPrimaryWeapon(entity player) {
  int slot = WEAPON_INVENTORY_SLOT_PRIMARY_0
...
void
function GiveRandomSecondaryWeapon(entity player) {
  int slot = WEAPON_INVENTORY_SLOT_PRIMARY_1
...
void
function GiveActualGungameWeapon(int index, entity player) {
  int slot = WEAPON_INVENTORY_SLOT_PRIMARY_0
...
void
function GiveRandomTac(entity player) {
  array < string > Weapons = [
    "mp_ability_grapple",
...
void
function GiveRandomUlt(entity player) {
  array < string > Weapons = [
...
void
function OnShipButtonUsed(entity panel, entity player, int useInputFlags) {
...
void
function CreateDropShipTriggerArea() {
...
void
function CreateShipRoomFallTriggers() {
  entity trigger = CreateEntity("trigger_cylinder")
...
void
function CreateFlowStateDeathBoxForPlayer(entity victim, entity attacker,
  var damageInfo) {
...
entity
function FlowState_CreateDeathBox(entity player, bool hasCard) {
  ...
  if (hasCard) {
    ...
    vector fallPos = restPos + < 0, 0, 54 >
      ...
      thread(void
        function (entity box, vector restPos, vector fallPos) {
...
void
function PlayerRestoreShieldsFIESTA(entity player, int shields) {
  if (IsValidPlayer(player) && IsAlive(player))
    player.SetShieldHealth(shielddd(shields, 0, player.GetShieldHealthMax()))
}

void
function PlayerRestoreHPFIESTA(entity player, int health) {
  if (IsValidPlayer(player) && IsAlive(player))
    player.SetHealth(health)
}

int
function shielddd(int value, int min, int max) {
...
void
function UpgradeShields(entity player, bool died) {
  if (!IsValid(player)) return
...
void
function KillStreakAnnouncer(entity player, bool died) {
...
#if SERVER
void
function GiveFlowstateOvershield(entity player, bool isOvershieldFromGround = false)
...
void
function GiveGungameWeapon(entity player) {
  //By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
...
entity
function CreateRingBoundary(LocationSettings location)
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
{
...
void
function AudioThread(entity circle, entity player, float radius)
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
{
...
void
function RingDamage(entity circle, float currentRadius)
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
{
  WaitFrame()
  ...
  while (IsValid(circle)) {
    foreach(dummy in GetNPCArray()) {
      ...
      if (playerDist > currentRadius) {
        dummy.TakeDamage(int(Deathmatch_GetOOBDamagePercent() / 100 * float(dummy.GetMaxHealth())), null, null, {
    ...
    foreach(player in GetPlayerArray_Alive()) {
      ...
      if (playerDist > currentRadius) {
        ...
        player.TakeDamage(int(Deathmatch_GetOOBDamagePercent() / 100 * float(player.GetMaxHealth())), null, null, {
          scriptType = DF_BYPASS_SHIELD | DF_DOOMED_HEALTH_LOSS,
          damageSourceId = eDamageSourceId.deathField
        })
      }
...
void
function PlayerRestoreHP(entity player, float health, float shields) {
  if (!IsValid(player)) return
...
void
function CharSelect(entity player)
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
{
  if (!FlowState_PROPHUNT()) {
    //Char select.
...
void
function Message(entity player, string text, string subText = "", float duration = 7.0, string sound = "")
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)//
{
  string sendMessage
...
int
function GetDamageOfPlayerWithMostDamage()
//Challenger's score
{
  int bestDamage = 0
  foreach(player in GetPlayerArray()) {
...
string
function PlayerWithMostDamageName()
//By Retículo Endoplasmático#5955 (CaféDeColombiaFPS)
{
...
float
function getkd(int kills, int deaths)
...
string
function ScoreboardFinal(bool fromConsole = false)
...
string
function ScoreboardFinalPROPHUNT(bool fromConsole = false)
//Este muestra el scoreboard completo
//Thanks marumaru（vesslanG）#3285
{
  array < PlayerInfo > playersInfo = []
...
int
function ComparePlayerInfo(PlayerInfo a, PlayerInfo b) {
...
void
function ResetPlayerStats(entity player) {
...
void
function PlayAnnounce(string sound) {
  foreach(player in GetPlayerArray_Alive()) {
...
bool
function ClientCommand_adminlogin(entity player, array < string > args) {
...
bool
function IsAdmin(entity player) {
...
bool
function CC_TDM_Weapon_Selector_Open(entity player, array < string > args) {
  //green highlight?
...
bool
function ClientCommand_MyFFAData(entity player, array < string > args) {
...
void
function ShowPlayerKD(entity player, string name) {
  // if(!IsValid(player)) 
  // return
...
float
function getcontrollerratio(int count, int kills)
//By michae\l/#1125 & Retículo Endoplasmático#5955
{
  float cCount
  int floorcCount
...
bool
function ClientCommand_FlowstateKick(entity player, array < string > args) {
...
bool
function ClientCommand_ControllerReport(entity player, array < string > args) {
...
bool
function ClientCommand_ControllerSummary(entity player, array < string > args) {
  if (!IsValid(player) || args.len() == 0)
    return false
...
bool
function ClientCommand_SpectateEnemies(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_Help(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_Say(entity player, array < string > args) {
...
bool
function ClientCommand_ShowLatency(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_GiveWeapon(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_NextRound(entity player, array < string > args) {
...
bool
function ClientCommand_adminnoclip(entity player, array < string > args) {
...
bool
function ClientCommand_CircleNow(entity player, array < string > args) {
...
bool
function ClientCommand_God(entity player, array < string > args) {
  if (!IsValid(player) || !IsAdmin(player))
    return false
...
bool
function ClientCommand_UnGod(entity player, array < string > args) {
  if (!IsValid(player) || !IsAdmin(player))
    return false
...
bool
function ClientCommand_Scoreboard(entity player, array < string > args) {
  if (!IsValid(player))
    return false
  ...
  Message(player,
    ...
    "Hosted by: " + GetOwnerName(), 4)

  return true
}

bool
function ClientCommand_ScoreboardPROPHUNT(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_RebalanceTeams(entity player, array < string > args) {
  if (!IsValid(player) || !IsAdmin(player) || args.len() == 0)
    return false
...
void
function CreateAnimatedLegend(asset a, vector pos, vector ang, int solidtype = 0, float size = 1.0) {
...
void
function AnimationTiming(entity legend, float cycle) {
...
bool
function ClientCommand_SaveCurrentWeapons(entity player, array < string > args) {
  entity weapon1
  entity weapon2
  string optics1
  string optics2
...
string
function modChecker(string weaponMods) {
...
void
function LoadCustomWeapon(entity player) {
  if (!IsValid(player)) return
...
bool
function ClientCommand_ResetSavedWeapons(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
void
function LoadCustomSkill(entity player) {
  if (!IsValid(player))
    return
...
bool
function ClientCommand_Maki_SaveCurSkill(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
bool
function ClientCommand_Maki_ResetSkills(entity player, array < string > args) {
  if (!IsValid(player))
    return false
...
void
function GivePlayerRandomCharacter(entity player) {
  if (!IsValid(player))
    return
...
void
function highlightKdMoreThan2(entity player) {
