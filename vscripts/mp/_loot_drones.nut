...
array<LootDroneData> function SpawnLootDrones( int numToSpawn )
{
	...
	{
		LootDroneData drone = LootDrones_SpawnLootDroneAtRandomPath()
		drones.append( drone )
...
array<entity> function LootDrones_GetRandomPath()
{
	...
	array<entity> path = file.dronePaths.getrandom()
	file.dronePaths.fastremovebyvalue( path )
	...
	OnThreadEnd(
		function() : ( data )
		{
			...
			if( IsValid( data.roller ) )
			{
				...
				// entity physics = CreatePropPhysics( data.roller.GetModelName(), data.roller.GetOrigin(), data.roller.GetAngles() )
				// data.roller.SetParent( physics )
				...
				// physics.SetVelocity( throwSpeed )
